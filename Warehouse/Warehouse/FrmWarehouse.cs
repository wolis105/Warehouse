...
namespace Warehouse
{
    public partial class FrmWarehouse : Form
    {
        ...
        private void tsmiUpdate_Click(object sender, EventArgs e)
        {
            if (this.lvwSelect.SelectedItems.Count > 0)
            {
                string whID = this.lvwSelect.SelectedItems[0].Text;
                FrmWarehouseUpdate f = new FrmWarehouseUpdate(whID);
                this.Hide();
                f.ShowDialog();
                this.Show();
        ...
        {
            this.lvwSelect.Items.Clear();
            string strSQL = "sp_SelectByName";
            using (SqlDataReader reader=db.ExecuteReader(strSQL,CommandType.StoredProcedure,new SqlParameter("WhName",this.cboName.Text.Trim())))
            {
                while (reader.Read())
                {
                    string WhID = reader.GetString(reader.GetOrdinal("WhID"));
